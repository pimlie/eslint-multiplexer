...
class Multiplexer {
  ...
  setOptions (cli) {
    Object.keys(defaultOptions).forEach((optionName) => {
      ...
      if (typeof defaultOptions[optionName] !== typeof optionValue) {
        // console.log(typeof defaultOptions[optionName], typeof optionValue)
      }
  ...
  resolveModulePath (modulePath) {
    ...
    for (let i = 0; i < paths.length; i++) {
      const path = resolve(paths[i], modulePath)
      if (existsSync(path)) {
        return path
      }
    }
  ...
  runFromCli (cli) {
    ...
    const p = readStdios.length ? Promise.all(readStdios) : Promise.resolve()
    p.then((stdioResults) => {
      const results = []
      ...
      if (!isPiped && this.options.format !== 'json') {
        try {
          ...
          let formatter = getFormatter(this.options.format)
